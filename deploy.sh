...
# $1 -> server IP addr
deploy_application() {
  ssh -i deploy/deploy_id_rsa isucon@$1 "\"./isucon7-qualifier/deploy/application.sh $2\""
}

# $1 -> server IP addr
deploy_database() {
  ssh -i deploy/deploy_id_rsa isucon@$1 "\"./isucon7-qualifier/deploy/database.sh $2\""
}
